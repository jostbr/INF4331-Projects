...
def mandelbrot_numpy(x_min, x_max, y_min, y_max, width, height, max_escape_iter = 1000):
    ...
    below_limit = np.zeros((height, width), dtype = np.bool)        # 2D-array to be mak for gridpoints outside M-set
    z = np.zeros((height, width), dtype = np.complex128)            # 2D-array to store evolution of sequence at each gridpoint
...
if (__name__ == "__main__"):
    x_min = -0.6; x_max = -0.45; y_min = -0.68; y_max = -0.5;     # Set parameters
    width = 2000; height = 2000; max_escape_iter = 5000;  # Set parameters

    time_start = time.clock()
    escape_iter_array = mandelbrot_numpy(x_min, x_max, y_min, y_max, width, height, max_escape_iter)    # Call to the NumPy version
    time_stop = time.clock()
    print("Execution took {0:.2f} seconds".format(time_stop - time_start))

    visualize_mandelbrot(escape_iter_array, x_min, x_max, y_min, y_max, width, height, max_escape_iter) # Visualize the results using matplotlib